...
func (s *Server) registerAPI(mux *http.ServeMux) {
	// Admin /api routes: Discord session cookie or admin Basic.
	mux.Handle("GET /api/status", s.requireAdmin(http.HandlerFunc(s.handleStatus)))
	// Product term: listener / listen. Legacy /api/airs and /api/mappings stay as aliases.
	mux.Handle("GET /api/listens", s.requireAdmin(http.HandlerFunc(s.handleListMappings)))
	mux.Handle("POST /api/listens", s.requireAdmin(http.HandlerFunc(s.handleUpsertMapping)))
	mux.Handle("PATCH /api/listens/{channel}", s.requireAdmin(http.HandlerFunc(s.handlePatchMapping)))
	mux.Handle("DELETE /api/listens/{channel}", s.requireAdmin(http.HandlerFunc(s.handleDeleteMapping)))
	mux.Handle("GET /api/airs", s.requireAdmin(http.HandlerFunc(s.handleListMappings)))
	mux.Handle("POST /api/airs", s.requireAdmin(http.HandlerFunc(s.handleUpsertMapping)))
	mux.Handle("PATCH /api/airs/{channel}", s.requireAdmin(http.HandlerFunc(s.handlePatchMapping)))
	mux.Handle("DELETE /api/airs/{channel}", s.requireAdmin(http.HandlerFunc(s.handleDeleteMapping)))
	mux.Handle("GET /api/mappings", s.requireAdmin(http.HandlerFunc(s.handleListMappings)))
	mux.Handle("POST /api/mappings", s.requireAdmin(http.HandlerFunc(s.handleUpsertMapping)))
	mux.Handle("PATCH /api/mappings/{channel}", s.requireAdmin(http.HandlerFunc(s.handlePatchMapping)))
	mux.Handle("DELETE /api/mappings/{channel}", s.requireAdmin(http.HandlerFunc(s.handleDeleteMapping)))
	mux.Handle("GET /api/activity", s.requireAdmin(http.HandlerFunc(s.handleActivity)))
	mux.Handle("POST /api/resync", s.requireAdmin(http.HandlerFunc(s.handleResync)))
	mux.Handle("GET /api/discord/guilds", s.requireAdmin(http.HandlerFunc(s.handleDiscordGuilds)))
	mux.Handle("GET /api/discord/guilds/{guild}/channels", s.requireAdmin(http.HandlerFunc(s.handleDiscordChannels)))
	mux.Handle("GET /api/discord/guilds/{guild}/channels/{channel}/messages", s.requireAdmin(http.HandlerFunc(s.handleDiscordMessages)))
	mux.Handle("POST /api/discord/guilds/{guild}/channels/{channel}/messages", s.requireAdmin(http.HandlerFunc(s.handleDiscordSendMessage)))
	mux.Handle("POST /api/discord/reconnect", s.requireAdmin(http.HandlerFunc(s.handleDiscordReconnect)))
	mux.Handle("GET /api/settings/listen-messages", s.requireAdmin(http.HandlerFunc(s.handleGetListenMessages)))
	mux.Handle("PUT /api/settings/listen-messages", s.requireAdmin(http.HandlerFunc(s.handlePutListenMessages)))
	mux.Handle("GET /api/settings/air-messages", s.requireAdmin(http.HandlerFunc(s.handleGetListenMessages)))
	mux.Handle("PUT /api/settings/air-messages", s.requireAdmin(http.HandlerFunc(s.handlePutListenMessages)))
	mux.Handle("GET /api/settings/picture-listen-messages", s.requireAdmin(http.HandlerFunc(s.handleGetPictureListenMessages)))
	mux.Handle("PUT /api/settings/picture-listen-messages", s.requireAdmin(http.HandlerFunc(s.handlePutPictureListenMessages)))
