...
**For:** next Cursor agent / maintainer  
**From:** Phase 0–6 arc (DEV verified + Phase 6 Admin polish)  
**Date:** 2026-09-08  
**Branch:** `master`

Phase 7 was **native deploy to a Linux VPS (EU), no Docker**. Phase 6 product/Admin polish is in good shape; do not reopen Phase 6 unless asked.
...
- Plain HTML/CSS/JS Admin; SQLite; beginner-friendly comments; commit only when asked
...
| README / guide | Clear deploy walkthrough (EU VPS) |

Optional (ask first): public `/healthz` without Basic Auth for uptime checks — today only `/api/status` (auth’d) exists.
...
### Config needed on VPS

See `.env.example` / PLAN.md §7. Especially:

- `ADMIN_HOST` — `0.0.0.0` direct, or `127.0.0.1` behind reverse proxy  
- `ADMIN_PORT` — default **50770**  
- `YOUTUBE_REDIRECT_URL` — may need a **production** OAuth redirect if re-authing on the VPS (DEV still uses `http://localhost:50770/oauth/callback`)  
- `DATABASE_PATH` — keep under a durable `data/` directory  
- `RESYNC_INTERVAL_HOURS` — leave `0` unless background scans are wanted  
...
5. **Proxy (optional)** — TLS + Basic Auth still on Subotto (or proxy auth — ask first).  
...
| Item | Notes |
|------|--------|
| Announce notice **preview** in Admin | Optional leftover Phase 6 polish — only if requested |
...
- [ ] Cross-compile from Windows and run on Linux VPS  
...
- [ ] Optional reverse proxy documented if TLS in front is wanted  

Then Subotto runs as a native binary on the operator’s own VPS.

---

**Note:** Phase 6 closed for product work. Phase 7 is ops — native binary, simple deploy. Keep it that way.
